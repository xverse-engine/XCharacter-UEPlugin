...
void URuntimeAudioImporterLibrary::ImportAudioFromFile(const FString& FilePath, ERuntimeAudioFormat AudioFormat)
{
		...
		AsyncTask(ENamedThreads::AnyBackgroundHiPriTask, [WeakThis = MakeWeakObjectPtr(this), FilePath, AudioFormat]()
		{
			...
			else
			{
				UE_LOG(LogRuntimeAudioImporter, Error,
				       TEXT(
					       "Failed to import audio from file '%s' because the RuntimeAudioImporterLibrary object has been destroyed"
				       ), *FilePath);
			}
		});
		return;
	...
	AudioFormat = AudioFormat == ERuntimeAudioFormat::Auto
		              ? (PossibleFormats.Num() == 0 ? ERuntimeAudioFormat::Invalid : PossibleFormats[0])
		              : AudioFormat;
...
UImportedSoundWave* URuntimeAudioImporterLibrary::ImportAudioFromBufferWithReturn(TArray<uint8> AudioData,
	ERuntimeAudioFormat AudioFormat)
{
	return ImportAudioWithReturn(TArray64<uint8>(MoveTemp(AudioData)), AudioFormat);
}
...
void URuntimeAudioImporterLibrary::ImportAudioFromBuffer(TArray64<uint8> AudioData, ERuntimeAudioFormat AudioFormat)
{
	if (IsInGameThread())
	{
		AsyncTask(ENamedThreads::AnyBackgroundHiPriTask,
		          [WeakThis = MakeWeakObjectPtr(this), AudioData = MoveTemp(AudioData), AudioFormat]() mutable
		          {
			          if (WeakThis.IsValid())
			          {
			          ...
			          else
			          {
				          UE_LOG(LogRuntimeAudioImporter, Error,
				                 TEXT(
					                 "Failed to import audio from buffer because the RuntimeAudioImporterLibrary object has been destroyed"
				                 ));
			          }
		          });
		return;
...
UImportedSoundWave* URuntimeAudioImporterLibrary::ImportAudioWithReturn(TArray64<uint8> AudioData,
                                                                        ERuntimeAudioFormat AudioFormat)
{
	if (IsInGameThread())
	{
	...
	if (!IsInGameThread())
	{
		AsyncTask(ENamedThreads::GameThread,
		          [WeakThis = MakeWeakObjectPtr(this), DecodedAudioInfo = MoveTemp(DecodedAudioInfo)]() mutable
		          {
			          if (WeakThis.IsValid())
			          {
			          ...
			          else
			          {
				          UE_LOG(LogRuntimeAudioImporter, Error,
				                 TEXT(
					                 "Unable to import audio from decoded info '%s' because the RuntimeAudioImporterLibrary object has been destroyed"
				                 ), *DecodedAudioInfo.ToString());
			          }
		          });
		return nullptr;
	}
...
void URuntimeAudioImporterLibrary::ImportAudioFromRAWFile(const FString& FilePath, ERuntimeRAWAudioFormat RAWFormat,
                                                          int32 SampleRate, int32 NumOfChannels)
{
#if WITH_RUNTIMEAUDIOIMPORTER_FILEOPERATION_SUPPORT
	if (IsInGameThread())
	{
		AsyncTask(ENamedThreads::AnyBackgroundHiPriTask,
		          [WeakThis = MakeWeakObjectPtr(this), FilePath, RAWFormat, SampleRate, NumOfChannels]()
		          {
			          if (WeakThis.IsValid())
			          {
			          ...
			          else
			          {
				          UE_LOG(LogRuntimeAudioImporter, Error,
				                 TEXT(
					                 "Failed to import RAW audio from file '%s' because the RuntimeAudioImporterLibrary object has been destroyed"
				                 ), *FilePath);
			          }
		          });
		return;
...
void URuntimeAudioImporterLibrary::ImportAudioFromRAWBuffer(TArray<uint8> RAWBuffer, ERuntimeRAWAudioFormat RAWFormat,
                                                            int32 SampleRate, int32 NumOfChannels)
{
	ImportAudioFromRAWBuffer(TArray64<uint8>(MoveTemp(RAWBuffer)), RAWFormat, SampleRate, NumOfChannels);
}

void URuntimeAudioImporterLibrary::ImportAudioFromRAWBuffer(TArray64<uint8> RAWBuffer, ERuntimeRAWAudioFormat RAWFormat,
                                                            int32 SampleRate, int32 NumOfChannels)
{
		...
		case ERuntimeRAWAudioFormat::Int8:
			{
				...
				FRAW_RuntimeCodec::TranscodeRAWData<int8, float>(reinterpret_cast<int8*>(ByteDataPtr), NumOfSamples,
				                                                 Float32DataPtr);
				break;
			}
		...
		case ERuntimeRAWAudioFormat::Int16:
			{
				...
				FRAW_RuntimeCodec::TranscodeRAWData<int16, float>(reinterpret_cast<int16*>(ByteDataPtr), NumOfSamples,
				                                                  Float32DataPtr);
				break;
			}
		case ERuntimeRAWAudioFormat::UInt16:
			{
				...
				FRAW_RuntimeCodec::TranscodeRAWData<uint16, float>(reinterpret_cast<uint16*>(ByteDataPtr), NumOfSamples,
				                                                   Float32DataPtr);
				break;
			}
		case ERuntimeRAWAudioFormat::UInt32:
			{
				...
				FRAW_RuntimeCodec::TranscodeRAWData<uint32, float>(reinterpret_cast<uint32*>(ByteDataPtr), NumOfSamples,
				                                                   Float32DataPtr);
				break;
			}
		case ERuntimeRAWAudioFormat::Int32:
			{
				...
				FRAW_RuntimeCodec::TranscodeRAWData<int32, float>(reinterpret_cast<int32*>(ByteDataPtr), NumOfSamples,
				                                                  Float32DataPtr);
				break;
			}
		case ERuntimeRAWAudioFormat::Float32:
			{
				...
				Float32DataPtr = static_cast<float*>(FMemory::Memcpy(FMemory::Malloc(ByteDataSize), ByteDataPtr,
				                                                     ByteDataSize));
				break;
			}
		}
	...
	ImportAudioFromFloat32Buffer(FRuntimeBulkDataBuffer<float>(Float32DataPtr, NumOfSamples), SampleRate,
	                             NumOfChannels);
}

void URuntimeAudioImporterLibrary::ConvertRegularToImportedSoundWave(USoundWave* SoundWave,
                                                                     TSubclassOf<UImportedSoundWave>
                                                                     ImportedSoundWaveClass,
                                                                     const
                                                                     FOnRegularToAudioImporterSoundWaveConvertResult&
                                                                     Result)
{
	ConvertRegularToImportedSoundWave(SoundWave, ImportedSoundWaveClass,
	                                  FOnRegularToAudioImporterSoundWaveConvertResultNative::CreateLambda(
		                                  [Result](bool bSucceeded, UImportedSoundWave* ImportedSoundWave)
		                                  {
			                                  Result.ExecuteIfBound(bSucceeded, ImportedSoundWave);
		                                  }));
}

bool URuntimeAudioImporterLibrary::TryToRetrieveSoundWaveData(USoundWave* SoundWave,
                                                              FDecodedAudioStruct& OutDecodedAudioInfo)
{
	...
	if (SoundWave->RawPCMData && SoundWave->RawPCMDataSize > 0)
	{
		...
		FRAW_RuntimeCodec::TranscodeRAWData<int16, float>(reinterpret_cast<int16*>(SoundWave->RawPCMData),
		                                                  SoundWave->RawPCMDataSize, TempFloatBuffer);
		...
		if (NumOfSamples > 0)
		{
			...
			OutDecodedAudioInfo.SoundWaveBasicInfo.Duration = static_cast<float>(OutDecodedAudioInfo.PCMInfo.
				PCMNumOfFrames) / SoundWave->GetSampleRateForCurrentPlatform();
	...
	{
		UE_LOG(LogRuntimeAudioImporter, Error,
		       TEXT("Failed to get AudioDevice for converting SoundWave to ImportedSoundWave"));
		return false;
	}
	...
	FByteBulkData* Bulk = SoundWave->GetCompressedData(RuntimeFormat,
	                                                   SoundWave->GetPlatformCompressionOverridesForCurrentPlatform());
	if (Bulk)
	{
	...
	UE_LOG(LogRuntimeAudioImporter, Error,
	       TEXT("Failed to convert SoundWave to ImportedSoundWave because SoundWave has no valid audio data"));
	return false;
}

UImportedSoundWave* URuntimeAudioImporterLibrary::ConvertRegularToImportedSoundWave(USoundWave* SoundWave)
{
	if (!IsValid(SoundWave))
	{
		UE_LOG(LogRuntimeAudioImporter, Error,
		       TEXT("Failed to convert SoundWave to ImportedSoundWave because SoundWave is nullptr"));
		return nullptr;
	}
	...
	{
		UE_LOG(LogRuntimeAudioImporter, Error,
		       TEXT("Failed to convert SoundWave to ImportedSoundWave because failed to retrieve SoundWave data"));
		return nullptr;
	}

	UImportedSoundWave* ImportedSoundWave = NewObject<UImportedSoundWave>(
		(UObject*)GetTransientPackage(), UImportedSoundWave::StaticClass());
	if (!ImportedSoundWave)
	{
		UE_LOG(LogRuntimeAudioImporter, Error,
		       TEXT(
			       "Failed to create an imported sound wave from class for converting SoundWave to ImportedSoundWave"
		       ));
		return nullptr;
	}
...
void URuntimeAudioImporterLibrary::ConvertRegularToImportedSoundWave(USoundWave* SoundWave,
                                                                     ...
                                                                     ImportedSoundWaveClass,
                                                                     const
                                                                     FOnRegularToAudioImporterSoundWaveConvertResultNative
                                                                     & Result)
{
	if (IsInGameThread())
	{
	...
	{
		UE_LOG(LogRuntimeAudioImporter, Error,
		       TEXT("Failed to convert SoundWave to ImportedSoundWave because ImportedSoundWaveClass is nullptr"));
		ExecuteResult(false, nullptr);
		return;
	}
	...
	AsyncTask(ENamedThreads::GameThread,
	          [ExecuteResult, ImportedSoundWaveClass, DecodedAudioInfo = MoveTemp(DecodedAudioInfo)]() mutable
	          {
		          UImportedSoundWave* ImportedSoundWave = NewObject<UImportedSoundWave>(
			          (UObject*)GetTransientPackage(), ImportedSoundWaveClass);
		          if (!ImportedSoundWave)
		          {
			          UE_LOG(LogRuntimeAudioImporter, Error,
			                 TEXT(
				                 "Failed to create an imported sound wave from %s class for converting SoundWave to ImportedSoundWave"
			                 ), *ImportedSoundWaveClass->GetName());
			          ExecuteResult(false, nullptr);
			          return;
		          }
		          ...
		          *Handle = ImportedSoundWave->OnPopulateAudioStateNative.AddLambda(
			          [Handle, ExecuteResult, ImportedSoundWave]()
			          {
...
bool URuntimeAudioImporterLibrary::ResampleAndMixChannelsInDecodedInfo(FDecodedAudioStruct& DecodedAudioInfo,
                                                                       uint32 NewSampleRate, uint32 NewNumOfChannels)
{
	if (DecodedAudioInfo.SoundWaveBasicInfo.SampleRate <= 0 || NewSampleRate <= 0)
	{
		UE_LOG(LogRuntimeAudioImporter, Error,
		       TEXT("Unable to resample audio data because the sample rate is invalid (Current: %d, New: %d)"),
	...
	{
		UE_LOG(LogRuntimeAudioImporter, Error,
		       TEXT("Unable to mix audio data because the number of channels is invalid (Current: %d, New: %d)"),
		       DecodedAudioInfo.SoundWaveBasicInfo.NumOfChannels, NewNumOfChannels);
		return false;
	}

	if (NewSampleRate == DecodedAudioInfo.SoundWaveBasicInfo.SampleRate && NewNumOfChannels == DecodedAudioInfo.
		SoundWaveBasicInfo.NumOfChannels)
	{
		UE_LOG(LogRuntimeAudioImporter, Log, TEXT("No need to resample or mix audio data"));
		return true;
	}

	Audio::FAlignedFloatBuffer WaveData(DecodedAudioInfo.PCMInfo.PCMData.GetView().GetData(),
	                                    DecodedAudioInfo.PCMInfo.PCMData.GetView().Num());

	// Resampling if needed
	if (NewSampleRate != DecodedAudioInfo.SoundWaveBasicInfo.SampleRate)
	{
		Audio::FAlignedFloatBuffer ResamplerOutputData;
		if (!FRAW_RuntimeCodec::ResampleRAWData(WaveData, DecodedAudioInfo.SoundWaveBasicInfo.NumOfChannels,
		                                        DecodedAudioInfo.SoundWaveBasicInfo.SampleRate, NewSampleRate,
		                                        ResamplerOutputData))
		{
			UE_LOG(LogRuntimeAudioImporter, Error,
			       TEXT("Unable to resample audio data to the sound wave's sample rate. Resampling failed"));
			return false;
		}
		WaveData = MoveTemp(ResamplerOutputData);
		DecodedAudioInfo.SoundWaveBasicInfo.SampleRate = NewSampleRate;
		UE_LOG(LogRuntimeAudioImporter, Log, TEXT("Audio data has been resampled to the desired sample rate '%d'"),
		       NewSampleRate);
	}
	...
	if (NewNumOfChannels != DecodedAudioInfo.SoundWaveBasicInfo.NumOfChannels)
	{
		...
		if (!FRAW_RuntimeCodec::MixChannelsRAWData(WaveData, NewSampleRate,
		                                           DecodedAudioInfo.SoundWaveBasicInfo.NumOfChannels, NewNumOfChannels,
		                                           WaveDataTemp))
		{
			UE_LOG(LogRuntimeAudioImporter, Error,
			       TEXT("Unable to mix audio data to the sound wave's number of channels. Mixing failed"));
			return false;
		}
		WaveData = MoveTemp(WaveDataTemp);
		DecodedAudioInfo.SoundWaveBasicInfo.NumOfChannels = NewNumOfChannels;
		UE_LOG(LogRuntimeAudioImporter, Log, TEXT("Audio data has been mixed to the desired number of channels '%d'"),
		       NewNumOfChannels);
	}
...
void URuntimeAudioImporterLibrary::ImportAudioFromFloat32Buffer(FRuntimeBulkDataBuffer<float>&& PCMData,
                                                                int32 SampleRate, int32 NumOfChannels)
{
	FDecodedAudioStruct DecodedAudioInfo;
	{
...
bool URuntimeAudioImporterLibrary::DecodeAudioData(FEncodedAudioStruct&& EncodedAudioInfo,
                                                   FDecodedAudioStruct& DecodedAudioInfo)
{
	...
	for (FBaseRuntimeCodec* RuntimeCodec : RuntimeCodecs)
	{
		...
		{
			UE_LOG(LogRuntimeAudioImporter, Error, TEXT("Something went wrong while decoding '%s' audio data"),
			       *UEnum::GetValueAsString(EncodedAudioInfo.AudioFormat));
			continue;
		}
		return true;
	}
	...
	UE_LOG(LogRuntimeAudioImporter, Error,
	       TEXT("Failed to decode the audio data because the codec for the format '%s' was not found"),
	       *UEnum::GetValueAsString(EncodedAudioInfo.AudioFormat));
	return false;
}

bool URuntimeAudioImporterLibrary::EncodeAudioData(FDecodedAudioStruct&& DecodedAudioInfo,
                                                   FEncodedAudioStruct& EncodedAudioInfo, uint8 Quality)
{
	if (EncodedAudioInfo.AudioFormat == ERuntimeAudioFormat::Auto || EncodedAudioInfo.AudioFormat ==
		ERuntimeAudioFormat::Invalid)
	{
	...
	for (FBaseRuntimeCodec* RuntimeCodec : RuntimeCodecs)
	{
		...
		{
			UE_LOG(LogRuntimeAudioImporter, Error, TEXT("Something went wrong while encoding '%s' audio data"),
			       *UEnum::GetValueAsString(EncodedAudioInfo.AudioFormat));
			continue;
		}
		return true;
	...
	UE_LOG(LogRuntimeAudioImporter, Error,
	       TEXT("Failed to encode the audio data because the codec for the format '%s' was not found"),
	       *UEnum::GetValueAsString(EncodedAudioInfo.AudioFormat));
	return false;
}
...
void URuntimeAudioImporterLibrary::OnResult_Internal(UImportedSoundWave* ImportedSoundWave, ERuntimeImportStatus Status)
{
	...
	if (!IsInGameThread())
	{
		AsyncTask(ENamedThreads::GameThread, [WeakThis = MakeWeakObjectPtr(this), ImportedSoundWave, Status]()
		{
			...
			else
			{
				UE_LOG(LogRuntimeAudioImporter, Error,
				       TEXT(
					       "Unable to broadcast the result of the import because the RuntimeAudioImporterLibrary object has been destroyed"
				       ));
			}
		});
		return;
	...
	{
		UE_LOG(LogRuntimeAudioImporter, Error,
		       TEXT("You did not bind to the delegate to get the result of the import"));
	}
}
